...
def generate_sketchy_url():
    ...
    if filetypes[-1] == ending_filetype:
        # Replace the last random filetype with something other than the ending filetype.
        filetypes[-1] = random.choice(list(set(sketchy_data.FILETYPES) - set(ending_filetype)))
    ...
    if random.random() < sketchy_data.LONG_RANDOM_STIRNG_CHANCE:
        sketchy_filename.insert(0, generate_random_string(random.randint(5, 20)))
